...
export const BuyPackage = async (req, res) => {
    try {
        ...
        if(user.balance<current_package.packageValue){
...
export const GetPackagesByUser = async (req, res) => {
    try {
        ...
        const all_packages = await BuyPackages.findAll({where: {
...
export const GetAllPackages = async (req, res) => {
    try {
        ...
        const all_packages = await Packages.findAll().then().catch(err => {
            console.log(err);
        });
        res.status(200).json({packages:all_packages, msg: "Success" });
    } catch (error) {
